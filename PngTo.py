...
if 'TEMPLATE_DIR' in os.environ:
    ...
    templates_folder = r"C:\Users\Professional\PycharmProjects\DownloadMusic\tmp"
...
def process_convertion():
    ...
    for import_key, import_value in import_formats.items():
        ...
        input_file = os.path.join(templates_folder, f"DM.png")
        print(input_file)
        if not os.path.exists(input_file):
            continue

        for export_key, export_value in export_formats.items():
            output_file = os.path.join(templates_folder, f"convert-png-to-{export_key}.{export_key}")
